...
{
    public static ParameterView ApplyMagicCasting(this ParameterView parameters, IComponent component)
    {
        var toSet = component.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance).Select(p => new
        {
            PropertyInfo = p,
            Attribute = p.GetCustomAttribute<AllowMagicCastingAttribute>()
